...
/*   Updated: 2024/09/05 13:07:31 by louismdv         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_threads(t_data *data)
{
	...
	if (pthread_create(&monitoring, NULL, &end_monitoring, data) != 0)
		return (destroy_threads(data),
			printf("pthread create error: monitoring "), EXIT_FAILURE);
	i = 0;
	while (i < data->table.num_of_philos)
	{
		if (pthread_create(&data->philos[i].thread, NULL, &routine,
				&data->philos[i]) != 0)
			return (destroy_threads(data),
				print_message("pthread_create error: ",
					data->philos, data->philos[i].id), EXIT_FAILURE);
		i++;
	}
